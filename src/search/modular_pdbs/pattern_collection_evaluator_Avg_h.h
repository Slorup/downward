#ifndef MODULAR_PDBS_PATTERN_COLLECTION_EVALUATOR_AVG_H_H
#define MODULAR_PDBS_PATTERN_COLLECTION_EVALUATOR_AVG_H_H
...
class PatterCollectionEvaluator_Avg_H : public PatternCollectionEvaluator {
	int time_limit=20;
...
//  std::shared_ptr<State_Avg_H> open_list;
  
  public:
  
  virtual void initialize(std::shared_ptr<AbstractTask> task) override;
	explicit PatterCollectionEvaluator_Avg_H(const options::Options &options);
