#include "eager_inteleaved_search.h"
...
void EagerSearchInterleaved::print_statistics() const {
...
pair<SearchNode, bool> EagerSearchInterleaved::fetch_next_node() {
...
void EagerSearchInterleaved::reward_progress() {
    // Boost the "preferred operator" open lists somewhat whenever
    // one of the heuristics finds a state with a new best h value.
    open_list->boost_preferred();
}

void EagerSearchInterleaved::dump_search_space() const {
    search_space.dump();
}

void EagerSearchInterleaved::start_f_value_statistics(EvaluationContext &eval_context) {
    if (f_evaluator) {
...
void EagerSearchInterleaved::update_f_value_statistics(const SearchNode &node) {
    if (f_evaluator) {
        /*
...
static SearchEngine *_parse(OptionParser &parser) {
    ...
    EagerSearchInterleaved *engine = nullptr;
    if (!parser.dry_run()) {
        opts.set<bool>("mpd", false);
        engine = new EagerSearchInterleaved(opts);
    }

    return engine;
}

static SearchEngine *_parse_astar_interleaved(OptionParser &parser) {
    parser.document_synopsis(
        "A* search (eager_interleaved)",
        "A* is a special case of eager_interleaved best first search that uses g+h "
        "as f-function. "
        "We break ties using the evaluator. Closed nodes are re-opened.");
    parser.document_note(
        "mpd option",
        "This option is currently only present for the A* algorithm and not "
        "for the more general eager_interleaved search, "
        "because the current implementation of multi-path depedence "
        "does not support general open lists.");
    parser.document_note(
        "Equivalent statements using general eager_interleaved search",
        "\n```\n--search astar_interleaved(evaluator)\n```\n"
        "is equivalent to\n"
        "```\n--heuristic h=evaluator\n"
        "--search eager_interleaved(tiebreaking([sum([g(), h]), h], unsafe_pruning=false),\n"
...
static SearchEngine *_parse_greedy(OptionParser &parser) {
    parser.document_synopsis("Greedy search (eager_interleaved)", "");
    parser.document_note(
        "Open list",
        "In most cases, eager_interleaved greedy best first search uses "
    ...
    parser.document_note(
        "Equivalent statements using general eager_interleaved search",
        "\n```\n--heuristic h2=eval2\n"
        "--search eager_interleaved_greedy([eval1, h2], preferred=h2, boost=100)\n```\n"
        "is equivalent to\n"
        "```\n--heuristic h1=eval1 --heuristic h2=eval2\n"
        "--search eager_interleaved(alt([single(h1), single(h1, pref_only=true), single(h2), \n"
        "                    single(h2, pref_only=true)], boost=100),\n"
        "               preferred=h2)\n```\n"
        "------------------------------------------------------------\n"
        "```\n--search eager_interleaved_greedy([eval1, eval2])\n```\n"
        "is equivalent to\n"
        "```\n--search eager_interleaved(alt([single(eval1), single(eval2)]))\n```\n"
        "------------------------------------------------------------\n"
        "```\n--heuristic h1=eval1\n"
        "--search eager_interleaved_greedy(h1, preferred=h1)\n```\n"
        "is equivalent to\n"
        "```\n--heuristic h1=eval1\n"
        "--search eager_interleaved(alt([single(h1), single(h1, pref_only=true)]),\n"
        "               preferred=h1)\n```\n"
        "------------------------------------------------------------\n"
        "```\n--search eager_interleaved_greedy(eval1)\n```\n"
        "is equivalent to\n"
        "```\n--search eager_interleaved(single(eval1))\n```\n", true);
...
static Plugin<SearchEngine> _plugin("eager_interleaved", _parse);
static Plugin<SearchEngine> _plugin_astar_interleaved("astar_interleaved", _parse_astar_interleaved);
static Plugin<SearchEngine> _plugin_greedy("eager_interleaved_greedy", _parse_greedy);
}
