...
class RelativeScatterPlotReport(PlotReport):
    """
    Generate a scatter plot for a specific attribute.
    """
    def __init__(self, show_missing=True, get_category=None, sec_atr=None, **kwargs):
        """
        ...
        *get_category* can be a function that takes **two** runs
        ...
        if not self.attribute:
            logging.critical('RelativeScatterPlotReport needs exactly one attribute')
    ...
    def _set_scales(self, xscale, yscale):
        ...
        if self.xscale != self.yscale:
            logging.critical('RelativeScatterplots must use the same scale on both axes.')
    ...
    def _fill_categories(self, runs):
        ...
        for (domain, problem), runs in self.problem_runs.items():
            ...
            val2 = run2.get(self.sec_atr if self.sec_atr else self.attribute)
    ...
    def write(self):
        if not len(self.algorithms) == 2:
            logging.critical(
                'RelativeScatter plots need exactly 2 algorithms: %s' % self.algorithms)
